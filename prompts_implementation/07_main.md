# Промпт для имплементации: Main Module

## Задача
Реализовать главный модуль приложения с консольным интерфейсом.

## Файл
`src/main.py`

## Контекст
Это точка входа в приложение. Модуль координирует работу всех компонентов и предоставляет консольный интерфейс для пользователя.

## Зависимости от других модулей
- `llm_client.py` - должен быть реализован
- `mcp_handler.py` - должен быть реализован
- `rag/` - все модули должны быть реализованы
- `prompts/system_prompt.py` - должен быть реализован

## Требования к имплементации

### Функция `load_config(config_path: str) -> dict`
```
Реализуй загрузку YAML конфигурации:
...
def __init__(self) -> None:
    ...
    # 4. MCP Handler
    from mcp_handler import MCPHandler, MCPServerConfig
    servers = {}
...
#### `process_input(self, user_input: str) -> Optional[str]`
```
Реализуй обработку ввода:
...
#### `handle_command(self, command: str) -> Optional[str]`
```
...
def _check_ticket_status(self, ticket_id: str) -> str:
    ...
    from mcp_handler import ToolCallRequest
    ...
    if result.success:
        return f"Статус тикета {ticket_id}: {result.result}"
    return f"Ошибка: {result.error_message}"
```

#### `send_to_llm(self, message: str) -> str`
```
Реализуй отправку в LLM:
...
#### `process_tool_calls(self, llm_response: str) -> str`
```
Реализуй обработку tool calls:
...
#### `_search_knowledge_base(self, query: str) -> dict`
```
Реализуй локальный инструмент поиска:
...
## Добавь необходимые импорты в начало файла
```python
import os
import sys
...
- Все внутренние модули проекта

## Тестирование
После реализации проверь:
1. Запуск без ошибок: `python main.py`
2. Команда /help показывает справку
3. Команда /index индексирует документы (нужен запущенный Ollama)
4. Команда /clear очищает историю
5. Отправка сообщения в LLM (нужен API ключ Perplexity)
6. Обработка tool_call в ответах LLM
